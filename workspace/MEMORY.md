...
### 本地文件 → IMA 笔记同步规范（2026-04-03 确立）
凡在本地生成文档、方案、设计文档等文件，均须同步推送一份到 IMA 笔记：
- **触发条件**：本地生成 .md 格式的文档/方案/设计文件
- **操作**：同步调用 IMA OpenAPI `import_doc`，将文件内容创建为 IMA 笔记
- **例外**：HEARTBEAT.md、MEMORY.md 等系统配置文件不进 IMA
- **执行时机**：生成文件后立即同步，不等待
